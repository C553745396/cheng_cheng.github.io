...
dsy.add("0_0",["北京辖区","北京辖县"]);
dsy.add("0_1",["天津辖区","天津辖县"]);
dsy.add("0_2",["上海辖区","上海辖县"]);
dsy.add("0_3",["重庆辖区","重庆辖县","重庆辖市"]);
...
dsy.add("0_31",["香港岛","九龙","新界东","新界西"]);
dsy.add("0_32",["花地玛堂区","圣安多尼堂区","花王堂区","大堂区","望德堂区","风顺堂区"]);
...
function s_province(){
	...
	for(var i=0;i<ar.length;i++){
		city_html+="<div class='province'><label class='checkbox-inline' style='margin-left:20px;'><input type='checkbox' class='cityall' />"+ar[i]+"<span class='citycount' style='color:#ff6600'></span></label><ul></ul></div>";

	}
	$('.modal-body').append(city_html);
	s_city();
}

function s_city(){
	
	$('.modal-body .province').each(function(e){
		for(var i=0;i<dsy.Items["0_"+e].length;i++){
			$(this).find('ul').append("<li><label class='checkbox-inline'><input type='checkbox' class='city' style='margin-top:8px;' city='"+dsy.Items["0_"+e][i]+"'/>"+dsy.Items["0_"+e][i]+"</label></li>");
		}
